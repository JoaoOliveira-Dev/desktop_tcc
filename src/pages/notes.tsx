...
export default function PentestNotes() {
  ...
  const [folders, setFolders] = useState<string[]>([
    "Recon", "Exploitation", "Post-Exploitation", "Vulnerability Analysis",
  ]);
  ...
  useEffect(() => {
    ...
    const fetchNotes = async () => {
      const result = await window.electron.getNotes(Number(projectId));
      setNotes(result);
    };
    fetchNotes();
  }, [projectId]);
  ...
  async function handleAddNote() {
    ...
    if (selectedNote) {
      // Atualizar
      const updated = { ...selectedNote, title, content, folder };
    ...
    <div className="flex h-screen text-white">
      ...
      <div className="w-1/4 border-r border-gray-800 p-2 flex flex-col">
        <div className="flex gap-2 mb-2">
          <Button onClick={newNoteForm} className="w-full">+ Nova Nota</Button>
          <Dialog open={isNewFolderOpen} onOpenChange={setIsNewFolderOpen}>
            <DialogTrigger asChild>
              <Button variant="outline"><FolderPlus size={16} /></Button>
            </DialogTrigger>
            <DialogContent className="max-w-sm text-white">
              ...
              <DialogFooter className="mt-4 flex justify-end gap-2">
                <Button variant="ghost" onClick={() => setIsNewFolderOpen(false)}>Cancelar</Button>
          ...
          <Accordion type="multiple" className="w-full">
            {folders.map((f) => (
              <AccordionItem key={f} value={f}>
                ...
                <AccordionContent>
                  {notes.filter((n) => n.folder === f).map((note) => (
                    <div key={note.id} className="flex justify-between items-center p-2 rounded">
                      <div onClick={() => handleSelectNote(note)} className="cursor-pointer flex-1">
                        <p className="font-semibold">{note.title}</p>
                        <p className="text-xs text-gray-400">{note.content.slice(0, 50)}...</p>
                      </div>
